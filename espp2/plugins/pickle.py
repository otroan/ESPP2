...
from espp2.datamodels import Dividend, Wire, Fee, Transfer
...
def do_deposit(record, source):
    ...
    return Deposit(
        ...
        purchase_price=Amount(
            amountdate=record["date"], currency="ESPPUSD", value=record["vpd"]
        ),
        source=source,
    )
...
def do_reinvest(record, source):
    ...
    return Dividend_Reinv(
        ...
        description="",
        amount=Amount(
            amountdate=record["date"], currency="USD", value=-record["amount"]
        ),
        source=source,
    )
...
def do_trans(record, source):
    ...
    if "fee" in record:
        fee = NegativeAmount(
            amountdate=record["date"], currency="USD", value=-record["fee"]
        )
    else:
        fee = None
    return Sell(
...
def do_dividend(record, source):
    ...
    d = Dividend(
        ...
        symbol="CSCO",
        description="",
        amount_ps=PositiveAmount(
            amountdate=record["payDate"], currency="USD", value=record["amount"]
        ),
        source=source,
    )
...
def do_tax(record, source):
    ...
    return Tax(
        ...
        symbol="CSCO",
        description="",
        amount=NegativeAmount(amountdate=record["date"], currency="USD", value=-amount),
        source=source,
    )
...
def do_rsu(record, source):
    ...
    return Deposit(
        ...
        purchase_date=record["date"],
        purchase_price=Amount(
            amountdate=record["date"], currency="USD", value=record["vpd"]
        ),
        source=source,
    )
...
def read(pickle_file, filename="") -> Transactions:
    ...
    for key in sorted(p.rawData):
        ...
        try:
            ...
            raise ValueError(
                f"Error: Unexpected pickle-file record: {rectype} {record}"
            ) from e
