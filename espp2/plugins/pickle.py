...
def dividend_equals(r1, r2):
    return r1.type==EntryTypeEnum.DIVIDEND and r2.type==r1.type and r1.date==r2.date and r1.symbol==r2.symbol

def dividend_in_list(r0, records):
    for r in records:
        if dividend_equals(r0, r):
            return True
    return False
...
def read(pickle_file, filename='') -> Transactions:
    ...
    for key in sorted(p.rawData):
        ...
        try:
            ...
            if r:
                if dividend_in_list(r, records):
                    # fixes an observed fault that a dividend appeared twice in pickle generated by legacy espp
                    logger.warning('Duplicate dividend entry in pickle file, ignoring it: %s', record)
                else:
                    records.append(r)
