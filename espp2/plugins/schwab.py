        ...
        while True:
            ...
            while row[field("Date")] == "":
                ...
                data[-1]["subdata"].append(
                    {subheader[v].upper(): k for v, k in enumerate(row) if v != 0}
                )
                row = next(reader)
                subheader = None
...
def subdata(action, description, date, value):
    ...
    datefields = ["purchase_date", "subscription_date", "award_date", "vest_date"]
    numberfields = ["shares"]
    pricefields = [
        "purchase_price",
        "purchase_fmv",
...
def read(csv_file, filename="") -> Transactions:  # noqa: C901
        ...
        for price in pricefields:
            if price in newv:
                if action == "SELL" and price == "fee":
                    newv[price] = fixup_price(
                        newv["date"], "USD", newv[price], change_sign=True
                    )
                else:
