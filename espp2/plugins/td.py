...
def action_to_type(value):  # noqa: C901
...
def read(raw_data, filename=""):  # noqa: C901
    ...
    for e in csv_data:
        ...
        if action.startswith("Bought"):
        ...
        elif action.startswith("ORDINARY DIVIDEND") or action.startswith(
            "QUALIFIED DIVIDEND"
        ):
        ...
        elif action.startswith("W-8 WITHHOLDING") or action.startswith(
            "BACKUP WITHHOLDING"
        ):
            t = EntryTypeEnum.TAX
            amount = fixup_price(d, "USD", e["AMOUNT"])
        ...
        ) or action.startswith("WIRE OUTGOING"):
            ...
            amount = fixup_price(d, "USD", e["AMOUNT"])
            r = {"type": t, "date": d, "amount": amount, "description": action}
